...
public class Adapter_ChiTiet_BSCPhongBan extends RecyclerView.Adapter<Adapter_ChiTiet_BSCPhongBan.RecyclerViewHolder> {
    private List<T_KPI_PhongBan> data = new ArrayList<>();
    private List<T_KPI_PhongBan> temp = new ArrayList<>();
    private Unbinder unbinder;
    private Context mContext;
    View view;

    public Adapter_ChiTiet_BSCPhongBan(Context mContext, List<T_KPI_PhongBan> data) {
        this.data = data;
    ...
    public RecyclerViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        view = inflater.inflate(R.layout.cardview_item_detail_bsc_phong_ban, parent, false);
...
//        holder.txtTyLe.setText(format1.format(dblTyLe) + " %");
    ...
    public void filter(String keys) {
        ...
        if (charText.length() == 0) {
            ...
            for (T_KPI_PhongBan kpi_phongBan : temp) {
                if (String.valueOf(kpi_phongBan.getChiTieuCuThe()).toLowerCase(Locale.getDefault()).contains(charText)
                ) {
                    data.add(kpi_phongBan);
                }
            }
        }
